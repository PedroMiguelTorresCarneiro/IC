...
std::vector<cv::Mat> ImageDecoder::convertToColorChannels(const cv::Mat& grayscaleImage) {
    // Check if the input is a valid grayscale image
    if (grayscaleImage.empty() || grayscaleImage.type() != CV_8UC1) {
        std::cout << "Error: Invalid grayscale image." << std::endl;
        return std::vector<cv::Mat>();  // Return an empty vector if error
    }

    // Create three channels (Red, Green, Blue)
    cv::Mat redChannel = cv::Mat::zeros(grayscaleImage.size(), CV_8UC1);
    cv::Mat greenChannel = cv::Mat::zeros(grayscaleImage.size(), CV_8UC1);
    cv::Mat blueChannel = cv::Mat::zeros(grayscaleImage.size(), CV_8UC1);
    ...
    for (int row = 0; row < grayscaleImage.rows; ++row) {
        for (int col = 0; col < grayscaleImage.cols; ++col) {
            ...
            // Declare variables for red, green, and blue channels
            int red, green, blue;

            // Reverse engineer grayscale value to approximate RGB
            red = static_cast<int>(std::min(255.0, gray / 0.299));
            green = static_cast<int>(std::min(255.0, gray / 0.587));
            blue = static_cast<int>(std::min(255.0, gray / 0.114));

            // Clamp values to [0, 255] and cast back to uchar
            red = std::clamp(red, 0, 255);
            green = std::clamp(green, 0, 255);
            blue = std::clamp(blue, 0, 255);

            // Set the channel values
            redChannel.at<uchar>(row, col) = static_cast<uchar>(red);
            greenChannel.at<uchar>(row, col) = static_cast<uchar>(green);
            blueChannel.at<uchar>(row, col) = static_cast<uchar>(blue);
        }
    }

    // Return the channels as a vector (Red, Green, Blue)
    return {redChannel, greenChannel, blueChannel};
}

// Overloaded version that returns a single 3-channel Mat
cv::Mat ImageDecoder::convertToColor(const cv::Mat& grayscaleImage) {
    std::vector<cv::Mat> channels = convertToColorChannels(grayscaleImage);

    if (channels.empty()) {
        return cv::Mat(); // Return an empty Mat if conversion failed
    }
...
std::map<int, int> ImageDecoder::createFrequencyTable(const cv::Mat& image) {
    // Check if the image is valid
    if (image.empty()) {
    ...
    if (channels == 1) {
        // Grayscale image
        std::cout << "Creating frequency table for grayscale image..." << std::endl;
        ...
        // BGR image
        std::cout << "Creating frequency table for color image (BGR)..." << std::endl;

        // Calculate the frequency table for each channel
        for (int i = 0; i < image.rows; i++) {
            for (int j = 0; j < image.cols; j++) {
                cv::Vec3b pixel = image.at<cv::Vec3b>(i, j);
                frequencyTable[pixel[0]]++; // Blue channel
                frequencyTable[pixel[1]]++; // Green channel
                frequencyTable[pixel[2]]++; // Red channel
            }
        }
